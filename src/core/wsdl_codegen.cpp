    ...
    namespace {
        ...
        std::string ToVariableIdentifier(std::string_view text) {
            ...
            static const std::vector<std::string> keywords = {
                "class", "struct", "int", "double", "float", "long", "bool", "return", "namespace", "template",
                "operator", "string"
            };
        ...
        WsdlModel ParseWsdl(const std::string &xml, const std::string &service_name_override) {
            ...
            for (const XmlNode *operation_node: ChildrenLocal(*port_type, "operation")) {
                ...
                if (!model.elements.contains(operation.input_element) || !model.elements.contains(
        ...
        std::string GenerateHeader(const WsdlModel &model) {
            ...
            for (const Operation &operation: model.operations) {
                ...
                out << "    " << output.cpp_name << " " << operation.cpp_name << "(const " << input.cpp_name <<
                        "& request) const;\n";
        ...
        std::string GenerateSource(const WsdlModel &model) {
            ...
            out << "const std::string& " << class_name <<
                    "::Endpoint() const noexcept\n{\n    return endpoint_;\n}\n\n";

            for (const Operation &operation: model.operations) {
                const Element &input = model.elements.at(operation.input_element);
                const Element &output = model.elements.at(operation.output_element);
                out << output.cpp_name << " " << class_name << "::" << operation.cpp_name << "(const " << input.cpp_name
                        << "& request) const\n{\n";
                out << "    std::ostringstream body;\n";
                out << "    body << \"<?xml version=\\\"1.0\\\" encoding=\\\"utf-8\\\"?>\";\n";
                out <<
                        "    body << \"<soap:Envelope xmlns:soap=\\\"http://schemas.xmlsoap.org/soap/envelope/\\\" xmlns:tns=\\\""
                        << model.target_namespace << "\\\">\";\n";
                out << "    body << \"<soap:Body><tns:" << input.xml_name << ">\";\n";
                for (const Field &field: input.fields) {
                    out << "    body << \"<" << field.xml_name << ">\" << " << ToXmlValueExpression(
                        field, "request." + field.cpp_name) << " << \"</" << field.xml_name << ">\";\n";
                }
                out << "    body << \"</tns:" << input.xml_name << "></soap:Body></soap:Envelope>\";\n";
                out << "    std::string xml = SendSoapRequest(endpoint_, " << CppStringLiteral(operation.soap_action) <<
                        ", body.str());\n";
